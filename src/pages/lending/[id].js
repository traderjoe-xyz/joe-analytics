...
import { getMarket } from "app/core/api";
...
function LendingPage(props) {
  ...
  const SupplyText = withStyles({})(Typography);

  const BorrowText = withStyles({})(Typography);
  ...
  const sortedMarketDayDatas = marketDayDatas
    .map((data) => {
      return {
        date: data.date,
  ...
  const supplyAPY = decimalFormatter.format(
    parseFloat(market.supplyRate * 100).toFixed(2)
  );
  const borrowAPY = decimalFormatter.format(
    parseFloat(market.borrowRate * 100).toFixed(2)
  );
  const utilizationRate =
    (Number(market.totalBorrows) /
      (Number(market.cash) +
        Number(market.totalBorrows) -
        Number(market.reserves))) *
    100;
  const liquidityUSD =
  ...
  return (
    <AppShell>
      <Head>
        <title>Trader Joe Analytics</title>
      </Head>
        ...
        <Grid item xs={12} md={6}>
          ...
          >
            ...
            <ProgressBar
              ...
              value={(liquidityUSD / totalLiquidityUSD) * 100}
            />
          </Card>
        </Grid>
        <Grid item xs={12} md={6}>
          ...
          >
            ...
            <ProgressBar
              style={{ width: "100%", marginRight: "20px" }}
              value={utilizationRate < 100 ? utilizationRate : 100}
            />
          </Card>
        </Grid>
      </Grid>
